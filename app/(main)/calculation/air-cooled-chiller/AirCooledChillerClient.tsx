...
export default function AirCooledChillerClient({
    id,
    initialData,
}: {
    id: string | null;
    ...
    // Fallback: only fetch on the client if the server didn't already provide the data
    // (e.g. the short-lived access token was expired at SSR time). fetchWithAuth refreshes
    // the token and retries — same behaviour as before the SSR change.
    useEffect(() => {
