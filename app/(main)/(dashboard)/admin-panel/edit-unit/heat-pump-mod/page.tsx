...
export default function EditHeatPumpModPage() {
    ...
    const display = (value: string) => {
        const map: Record<string, string> = {
            "Select Heat Pump": "selectHeatPump",
            "Select Evaporator": "selectEvaporator",
            "Select Condenser": "selectCondenser",
            "Select Expansion Valve": "selectExpansionValve",
            "Select Reversing Valve": "selectReversingValve",
    ...
    const handleSave = async () => {
        ...
        if (!selectedMod) {
            showToast(t("pleaseSelectModeToEdit"), "error");
            setActiveTab("model");
            return;
        }
        if (evaporatorSpecsId === null || condenserSpecsId === null ||
        ...
        try {
            ...
            if (res.ok) {
                showToast(t("modeUpdated", { mode: selectedMod === "cooling" ? t("cooling") : t("heating") }), "success");
            } else {
                let msg = t("failedUpdateMode");
    ...
    const modelComplete =
        selectedHeatPumpId !== null && selectedMod !== "" &&
        evaporatorSpecsId !== null && condenserSpecsId !== null && expansionValveSpecsId !== null;
                ...
                <div className={styles.formSection}>
                    {activeTab === 'model' && (
                        <div className={styles.formGrid}>
                            ...
                            <div className={styles.formField}>
                                <label>{t("eer")}</label>
                                <input type="number" onWheel={(e) => e.currentTarget.blur()} className={styles.inputElement} value={eer} onChange={(e) => setEer(e.target.value)} disabled={selectedMod === 'heating'} />
                            </div>
                            <div className={styles.formField}>
                                <label>{t("cop")}</label>
                                <input type="number" onWheel={(e) => e.currentTarget.blur()} className={styles.inputElement} value={cop} onChange={(e) => setCop(e.target.value)} disabled={selectedMod === 'cooling'} />
                            </div>
                            <div className={styles.formField}>
                    ...
                    {activeTab === 'calc' && (
                        <div className={styles.formGrid}>
                            ...
                            <div className={styles.formField}>
                                ...
                                <input type="number" onWheel={(e) => e.currentTarget.blur()} className={styles.inputElement} value={evapIn} onChange={(e) => setEvapIn(e.target.value)} disabled={selectedMod === 'heating'} />
                            </div>
                            <div className={styles.formField}>
                                <label>{t("evapOutlet")}</label>
                                <input type="number" onWheel={(e) => e.currentTarget.blur()} className={styles.inputElement} value={evapOut} onChange={(e) => setEvapOut(e.target.value)} disabled={selectedMod === 'heating'} />
                            </div>
                            <div className={styles.formField}>
                                <label>{t("condInlet")}</label>
                                <input type="number" onWheel={(e) => e.currentTarget.blur()} className={styles.inputElement} value={condIn} onChange={(e) => setCondIn(e.target.value)} disabled={unitType === 'air_to_water' && selectedMod !== 'heating'} />
                            </div>
                            <div className={styles.formField}>
                                <label>{t("condOutlet")}</label>
                                <input type="number" onWheel={(e) => e.currentTarget.blur()} className={styles.inputElement} value={condOut} onChange={(e) => setCondOut(e.target.value)} disabled={unitType === 'air_to_water' && selectedMod !== 'heating'} />
                            </div>
                        </div>
                    )}
